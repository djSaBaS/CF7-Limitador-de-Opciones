-- Consulta responsable de crear la tabla principal del plugin respetando el prefijo dinámico.
CREATE TABLE IF NOT EXISTS `{prefix}cf7_option_limits` (
    -- Identificador único autoincremental de la regla.
    id INT AUTO_INCREMENT PRIMARY KEY,
    -- ID del formulario de Contact Form 7 asociado a la regla.
    form_id BIGINT NOT NULL,
    -- Nombre del campo dentro del formulario.
    field_name VARCHAR(100) NOT NULL,
    -- Valor exacto de la opción a limitar.
    option_value VARCHAR(255) NOT NULL,
    -- Indicador que determina si la opción debe ocultarse cuando alcance el límite.
    hide_exhausted TINYINT(1) NOT NULL DEFAULT 0,
    -- Número máximo de selecciones permitidas para la opción.
    max_count INT NOT NULL DEFAULT 1,
    -- Contador actual de selecciones acumuladas.
    current_count INT NOT NULL DEFAULT 0,
    -- Periodo de aplicación del límite.
    limit_period ENUM('none','hour','day','week') DEFAULT 'none',
    -- Fecha y hora del último reinicio automático asociado al periodo.
    limit_reset DATETIME NULL,
    -- Mensaje personalizado que se mostrará cuando la opción se agote.
    custom_message VARCHAR(255) DEFAULT NULL,
    -- Fecha de creación del registro.
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    -- Fecha de última actualización del registro.
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    -- Clave única que evita reglas duplicadas para la misma opción.
    UNIQUE KEY unique_limit (form_id, field_name, option_value)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Consulta responsable de crear la tabla de excepciones específicas respetando el prefijo dinámico.
